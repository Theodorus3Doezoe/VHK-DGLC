import {drones, dronesAnswers} from '../script/AircraftDB copy.js'
...
function askQuestion() {
    if (isRunning) {
        let index = Math.floor(Math.random() * drones.length)
        const randomSubArray = drones[index];
        const q = randomSubArray[Math.floor(Math.random() * randomSubArray.length)];
        const a = dronesAnswers[index]
