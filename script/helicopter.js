...
function askQuestion() {
    if (isRunning) {
        let index = Math.floor(Math.random() * helicopters.length)
        const randomSubArray = helicopters[index];
        const q = randomSubArray[Math.floor(Math.random() * randomSubArray.length)];
        const a = helicoptersAnswers[index][0]
        const r = helicoptersAnswers[index][1]
        const fof = helicoptersAnswers[index][2]
